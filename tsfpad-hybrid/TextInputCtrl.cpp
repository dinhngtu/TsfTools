...
HWND CTextInputCtrl::Create(HWND hwndParent) {
    ...
    if (!_hwnd) {
        WINERROR_PRINT(GetLastError(), L"CreateWindow(TEXTINPUTCTRL_CLASSNAME) failed");
    }
...
LRESULT CALLBACK CTextInputCtrl::s_WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_KEYDOWN:
        DPRINT(L"WM_KEYDOWN: wParam=0x%x, lParam=0x%x", wParam, lParam);
        ptic = GetThis(hwnd);
        if (ptic)
    ...
    case WM_LBUTTONDOWN:
        DPRINT(L"WM_LBUTTONDOWN: wParam=0x%x, lParam=0x%x", wParam, lParam);
        ptic = GetThis(hwnd);
        if (ptic)
                ...
                if (lSize) {
                    ...
                    if (psz) {
                        if (ImmGetCompositionString(himc, GCS_COMPSTR, psz, lSize)) {
                            ...
                            ptic = GetThis(hwnd);
                            if (ptic) {
                                ptic->_editor.UpdateComposition(
                                    psz,
                                    lDeltaStart,
                                    lCursorPos,
                                    prgAttr,
    ...
    case WM_CHAR:
        DPRINT(L"WM_CHAR: wParam=0x%x, lParam=0x%x", wParam, lParam);
        //
        // wParam is a character of the result string.
        //
