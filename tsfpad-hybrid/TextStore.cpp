...
STDAPI CTextStore::AdviseSink(REFIID riid, IUnknown *punk, DWORD dwMask) {
    ...
    if (!IsEqualGUID(riid, IID_ITextStoreACPSink)) {
        DPRINT(L"AdviseSink failed: TS_E_NOOBJECT");
        return TS_E_NOOBJECT;
    }

    HRESULT hr;
    if (FAILED(hr = punk->QueryInterface(IID_ITextStoreACPSink, (void **)&_pSink))) {
        HRESULT_PRINT(hr, L"punk->QI(IID_ITextStoreACPSink) failed");
        return E_NOINTERFACE;
    }
...
STDAPI CTextStore::RequestLock(DWORD dwLockFlags, HRESULT *phrSession) {
    DPRINT(
        L"dwLockFlags=0x%x (%s%s)",
        dwLockFlags,
        (dwLockFlags & TS_LF_READWRITE) == TS_LF_READWRITE ? L"READWRITE "
            : (dwLockFlags & TS_LF_READ)                   ? L"READ "
                                                           : L"",
        (dwLockFlags & TS_LF_SYNC) ? L"SYNC" : L"ASYNC");
    ...
    if (!_pSink) {
        DPRINT(L"RequestLock failed: E_UNEXPECTED (no sink)");
        return E_UNEXPECTED;
    }
...
STDAPI
CTextStore::QueryInsert(LONG acpInsertStart, LONG acpInsertEnd, ULONG cch, LONG *pacpResultStart, LONG *pacpResultEnd) {
    DPRINT(L"");
...
STDAPI CTextStore::GetSelection(ULONG ulIndex, ULONG ulCount, TS_SELECTION_ACP *pSelection, ULONG *pcFetched) {
    *pcFetched = 0;
    if ((ulCount > 0) && ((ulIndex == 0) || (ulIndex == TS_DEFAULT_SELECTION))) {
        ...
        *pcFetched = 1;
        DPRINT(L"acpStart=%d, acpEnd=%d", pSelection[0].acpStart, pSelection[0].acpEnd);
    } else {
        DPRINT(L"ulIndex=%d, ulCount=%d", ulIndex, ulCount);
    }
...
STDAPI CTextStore::GetText(
    LONG acpStart,
    LONG acpEnd,
    __out_ecount(cchPlainReq) WCHAR *pchPlain,
    ULONG cchPlainReq,
    ULONG *pcchPlainOut,
    ...
    if ((acpStart != acpEnd) && !_pEditor->GetText(acpStart, pchPlain, acpEnd - acpStart)) {
        DPRINT(L"GetText failed: _pEditor->GetText returned FALSE");
        return E_FAIL;
    }
...
STDAPI CTextStore::SetText(
    DWORD dwFlags,
    LONG acpStart,
    LONG acpEnd,
    __in_ecount(cch) const WCHAR *pchText,
    ULONG cch,
    TS_TEXTCHANGE *pChange) {
    DPRINT(L"acpStart=%d, acpEnd=%d, cch=%d, text=\"%.*s\"", acpStart, acpEnd, cch, cch, pchText);
    LONG acpRemovingEnd;

#ifdef HOOK_REQUESTLOCK
    if (cch > 0 && pchText[0] == L'q') {
        _fTrap = TRUE;
    }
#endif

    if (acpStart > (LONG)_pEditor->GetTextLength()) {
        DPRINT(L"SetText failed: E_INVALIDARG (acpStart out of range)");
        return E_INVALIDARG;
    }

    acpRemovingEnd = min(acpEnd, (LONG)_pEditor->GetTextLength() + 1);
    if (!_pEditor->RemoveText(acpStart, acpRemovingEnd - acpStart)) {
        DPRINT(L"SetText failed: _pEditor->RemoveText returned FALSE");
        return E_FAIL;
    }

    if (!_pEditor->InsertText(acpStart, pchText, cch)) {
        DPRINT(L"SetText failed: _pEditor->InsertText returned FALSE");
        return E_FAIL;
    }
...
STDAPI CTextStore::InsertEmbedded(
    DWORD dwFlags,
    LONG acpStart,
    LONG acpEnd,
...
STDAPI
CTextStore::RequestAttrsAtPosition(LONG acpPos, ULONG cFilterAttrs, const TS_ATTRID *paFilterAttrs, DWORD dwFlags) {
    PrepareAttributes(cFilterAttrs, paFilterAttrs);
    if (!_nAttrVals)
        return S_FALSE;
    return S_OK;
...
STDAPI CTextStore::RequestAttrsTransitioningAtPosition(
    LONG acpPos,
    ULONG cFilterAttrs,
...
STDAPI CTextStore::FindNextAttrTransition(
    LONG acpStart,
    LONG acpHalt,
    ULONG cFilterAttrs,
    const TS_ATTRID *paFilterAttrs,
...
STDAPI CTextStore::GetTextExt(TsViewCookie vcView, LONG acpStart, LONG acpEnd, RECT *prc, BOOL *pfClipped) {
    ...
    *pfClipped = FALSE;
    DPRINT(
        L"acpStart=%d, acpEnd=%d, rect={%d, %d, %d, %d}",
        acpStart,
        acpEnd,
        prc->left,
        prc->top,
...
STDAPI CTextStore::GetScreenExt(TsViewCookie vcView, RECT *prc) {
    ...
    ClientToScreen(_pEditor->GetWnd(), (POINT *)&prc->left);
    ClientToScreen(_pEditor->GetWnd(), (POINT *)&prc->right);
    DPRINT(L"rect={%d, %d, %d, %d}", prc->left, prc->top, prc->right, prc->bottom);
    return E_NOTIMPL;
}
...
STDAPI CTextStore::InsertTextAtSelection(
    DWORD dwFlags,
    __in_ecount(cch) const WCHAR *pchText,
    ULONG cch,
    LONG *pacpStart,
    ...
    if (!_pEditor->RemoveText(acpStart, acpEnd - acpStart)) {
        DPRINT(L"InsertTextAtSelection failed: _pEditor->RemoveText returned FALSE");
        return E_FAIL;
    }

    if (pchText && !_pEditor->InsertText(acpStart, pchText, cch)) {
        DPRINT(L"InsertTextAtSelection failed: _pEditor->InsertText returned FALSE");
        return E_FAIL;
    }
...
STDAPI CTextStore::InsertEmbeddedAtSelection(
    DWORD dwFlags,
    IDataObject *pDataObject,
    LONG *pacpStart,
    LONG *pacpEnd,
...
STDAPI CTextStore::AdviseMouseSink(ITfRangeACP *range, ITfMouseSink *pSink, DWORD *pdwCookie) {
    ...
    if (!_prgMouseSinks) {
        ...
        if (!_prgMouseSinks) {
            WINERROR_PRINT(GetLastError(), L"AdviseMouseSink: LocalAlloc failed");
        ...
        if (i == _nMouseSinks) {
            ...
            if (!pvNew) {
                WINERROR_PRINT(GetLastError(), L"AdviseMouseSink: LocalReAlloc failed");
                return E_OUTOFMEMORY;
            }
...
STDAPI CTextStore::OnUpdateComposition(ITfCompositionView *pComposition, ITfRange *pRangeNew) {
    DPRINT(L"pRangeNew=%p", pRangeNew);
    if (_pCurrentCompositionView) {
        _pCurrentCompositionView->Release();
        _pCurrentCompositionView = NULL;
    }
