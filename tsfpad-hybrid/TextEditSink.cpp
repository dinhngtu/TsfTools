...
STDAPI CTextEditSink::OnEndEdit(ITfContext *pic, TfEditCookie ecReadOnly, ITfEditRecord *pEditRecord) {
    ...
    if (pDispAttrProps) {
        IEnumTfRanges *pEnum;
        if (SUCCEEDED(pEditRecord->GetTextAndPropertyUpdates(
                TF_GTP_INCL_TEXT,
...
HRESULT CTextEditSink::_Advise(ITfContext *pic) {
    ...
    if (FAILED(hr = pic->QueryInterface(IID_ITfSource, (void **)&source))) {
        HRESULT_PRINT(hr, L"pic->QI(IID_ITfSource) failed");
        goto Exit;
    }

    if (FAILED(hr = source->AdviseSink(IID_ITfTextEditSink, (ITfTextEditSink *)this, &_dwEditCookie))) {
        HRESULT_PRINT(hr, L"source->AdviseSink failed");
        goto Exit;
    }
...
HRESULT CTextEditSink::_Unadvise() {
    ...
    if (FAILED(hr = _pic->QueryInterface(IID_ITfSource, (void **)&source))) {
        HRESULT_PRINT(hr, L"_pic->QI(IID_ITfSource) failed");
        goto Exit;
    }

    if (FAILED(hr = source->UnadviseSink(_dwEditCookie))) {
        HRESULT_PRINT(hr, L"source->UnadviseSink failed");
        goto Exit;
    }
