...
BOOL CTextEditor::InitTSF() {
    ...
    if (!_pTextStore) {
        DPRINT(L"new CTextStore failed");
        return FALSE;
    }

    HRESULT hr;
    if (FAILED(hr = g_pThreadMgr->CreateDocumentMgr(&_pDocumentMgr))) {
        HRESULT_PRINT(hr, L"g_pThreadMgr->CreateDocumentMgr failed");
        return FALSE;
    }

    if (FAILED(hr = _pDocumentMgr
                   ->CreateContext(g_TfClientId, 0, (ITextStoreACP *)_pTextStore, &_pInputContext, &_ecTextStore))) {
        HRESULT_PRINT(hr, L"_pDocumentMgr->CreateContext failed");
        return FALSE;
    }

    if (FAILED(hr = _pDocumentMgr->Push(_pInputContext))) {
        HRESULT_PRINT(hr, L"_pDocumentMgr->Push failed");
        return FALSE;
    }
    ...
    if (!_pTextEditSink) {
        DPRINT(L"new CTextEditSink failed");
        return FALSE;
    }

    if (FAILED(hr = _pTextEditSink->_Advise(_pInputContext))) {
        HRESULT_PRINT(hr, L"_pTextEditSink->_Advise failed");
    }
...
BOOL CTextEditor::UpdateComposition(
    LPCWSTR psz,
    LONG lDeltaStart,
    LONG lCursorPos,
    const BYTE *prgAttr,
    LONG lAttr,
    const LONG *prgClauseInfo,
    LONG lClauseInfo) {
...
void CTextEditor::SetCandidateForm() {
    ...
    if (himc) {
        ...
        if (_layout.RectFromCharPos(_nCompStart, &rcStart) && _layout.RectFromCharPos(_nCompEnd, &rcEnd) &&
