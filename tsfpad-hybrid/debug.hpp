...
#if !defined(ENABLE_LOG) && _DEBUG
#define ENABLE_LOG 1
#endif

#if !defined(ENABLE_DEBUG_LOG) && _DEBUG
#define ENABLE_DEBUG_LOG 1
#endif
...
static void _dprint(_In_ LPCWSTR func, _In_ int line, _In_ LPCWSTR fmt, Args... args) {
    ...
    StringCchPrintf(&buf[0], buf.size(), L"[%02d:%02d:%02d.%03d] ", st.wHour, st.wMinute, st.wSecond, st.wMilliseconds);
